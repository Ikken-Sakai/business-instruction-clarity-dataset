...
CONFIG = {
    ...
    'batch_size': 4,   # メモリ不足対策でさらに小さく
    'learning_rate': 2e-5,
    'num_epochs': 1,   # メモリ不足対策で1エポックのみ
    'weight_decay': 0.01,
    'warmup_steps': 5,  # テスト用に短く
...
def create_dataset():
    """
    HuggingFace Dataset形式に変換（テスト用）
    """
    print("データファイルを読み込み中...")
    train_data = load_jsonl('train_small.jsonl')
...
def main():
    ...
    plot_confusion_matrix(y_true, y_pred, 'figures/confusion_matrix_test.png')
    ...
    print("  - 可視化: OK")
    print("\n📝 次のステップ:")
    print("  1. 本番スクリプト（02_train_bert.py）を使用")
    print("  2. 全データで学習を実行（コマ3）")
    print("="*60)
